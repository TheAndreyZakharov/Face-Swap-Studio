...
from src.face_swap_studio.adapters.base import SwapAdapter
...
AdapterFactory = Callable[[], SwapAdapter]
...
def get_adapter(
    ...
    try:
        factory = _ADAPTER_FACTORIES[normalized_model_id]
    except KeyError as error:
        supported = ", ".join(supported_model_ids())

        raise ValueError(
            f"Для модели {normalized_model_id!r} не зарегистрирован адаптер. "
            f"Поддерживаются: {supported}"
        ) from error

    adapter = factory()

    if not isinstance(adapter, SwapAdapter):
        raise TypeError(
            f"Фабрика модели {normalized_model_id!r} вернула "
            f"некорректный объект: {type(adapter).__name__}."
        )
